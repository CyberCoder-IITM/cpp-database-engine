...
void BTreeFile::insertTuple(const Tuple &t) {
    ...
    if (root.header->size == 0 && root.children[0] != 1) {
        // Tree is empty: allocate the first leaf page and point the root at it.
        bufferPool.markDirty({name, root_id});
        pid.page = numPages++;
        root.children[0] = pid.page;
    } else {
        // Traverse from the root to the correct leaf, recording the path for
        // propagating splits back up if needed.
        while (true) {
    ...
    // Leaf is full: split it and propagate the new separator key upward.
    pid.page = numPages++;
    ...
    // Split propagated all the way to the root: grow the tree by one level.
    bufferPool.markDirty({name, root_id});
